...
- `SecureVectorCallbackHandler` — a `BaseCallbackHandler` that instruments
  LangGraph tool nodes (which execute through langchain-core's callback
  manager) with the three controls on every tool call:
